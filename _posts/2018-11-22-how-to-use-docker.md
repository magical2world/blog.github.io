...
软件开发最麻烦的事之一就是对于环境的被试。当你的程序在一台机器上顺利跑通的时候，这时你想将你的程序移植到其他的机器上，这时就会遇到重新配置环境的问题，如果两台机器的操作系统及配置都相差无几还好，否则就会有这样那样的问题。

而虚拟机允许在一种操作系统里运行另一种操作系统，因此面对上面的问题，只需在你的系统中安装虚拟机同样配置的虚拟机就能解决了，但是虚拟机具有资源占用多，冗余步骤多，启动慢等缺点。而Linux有另一种虚拟化技术Linux容器，它不是模拟一个完整的操作系统，而是对进程进行隔离，拥有启动快，资源占用少，体积小等优点。而docker正是最流行的Linux容器解决方案。
